...
public:
    static at::Tensor Get(const T &tilingData, const std::string &opName)
    {
        ...
        // is launched right after this call. The cache must therefore never hand
        // out a temporary tensor whose storage can be freed/reused before that
        // kernel completes (long chunked prefill exhausts the first slab and used
        // to fall back to a one-shot at::empty -> async MTE out-of-range, #769).
        // Instead, when the current fixed-address slab is full we append another
        // persistent slab and keep every slab alive for the process lifetime.
